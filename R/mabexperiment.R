...
#' @description Runs a multi-armed bandit (MAB) experiment, supporting TS, UCB, Gaussian Process-based variants, and monotonic and heterogeneous variants.
...
#' @param Timeout Seconds allowed for each truncated-sampling attempt in the monotonic fallback chain (optional, default is 5).
...
MABExperiment <- function(Valuations, Policy, TestX, NumIter, BatchSize,
                          ...
                          HeteroNoise = FALSE, Reset = NULL, Timeout = 5) {

  # Reset fallback counters (RNG-neutral)
  ResetDiagnostics()
